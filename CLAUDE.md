...
**Live `--dry-run` against Anthropic executed 2026-04-20** (closes Phase 6's last operator-followup gap). Running `npm run reviewer -- --dry-run` across the five seed articles produced 39 findings total, NPOV clean on every article; Luis judged the findings actionable and honest. Composed output lives at `/tmp/reviewer-smoke-seed.out` (operator-local only, not checked in). This satisfies the Phase 6 Done-when live-smoke criterion — the only pending items are now Phase 7 (deploy) and the post-Phase-6 issues tracked in `.chainlink/` (see below).

### Post-Phase-6 backlog (tracked in `.chainlink/`)

Chainlink was initialized this session with three issues sequencing the work after Phase 6:

- **#2 Reviewer: three-pane human review UI (anti-rubber-stamp)** — priority: high, status: next. Anti-rubber-stamp human review UI; three-pane layout (proposed change / reviewer finding / source web page) is a brainstorming starting point, not a locked design.
- **#1 Reviewer: edit-proposer with fresh-session consensus check** — priority: medium, blocked by #2. Separate agent consumes reviewer `Finding`s and emits typed edit candidates, vetted by a fresh-session consensus check. Motivated by the dry-run's ~30 of 39 findings being mechanically actionable.
- **#3 Phase 7: CI, Deploy & Round-Trip Verification** — priority: medium, blocked by #2. The existing `plans/implementation-plans/2026-04-20-mvp/phase_07.md` converted into a tracked issue.

**Sequencing decision: #2 ships before Phase 7.** Rationale: the dry-run showed ~8 findings per small article, so a single new-article PR carries 5–10 findings to triage. At that density, GitHub's default PR-comment UI plus manually-opened Wayback tabs is unsustainable and rubber-stamps within weeks. The review UI has to exist before the reviewer is wired into real PR workflow, or the reviewer's signal gets ignored in practice.
...
- **`clean-baseline` fixture's QID mismatch is intentional — do not "fix" it.** `tests/reviewer-fixtures/clean-baseline/subjects/jackie-fielder.yaml` uses `Q98157141`, which actually points at "Kieran Sweeney, a researcher" on live Wikidata, not Jackie Fielder. The live `--dry-run` correctly flags this as a source–subject mismatch, which happens to exercise a fourth failure mode (source-doesn't-match-subject) beyond the three the other fixtures target (coverage, support, NPOV). Integration tests pass regardless because they mock the LLM and network. Keeping the mismatch is a feature, not a cleanup target. See memory `clean-baseline-fixture-qid-intentional.md`.
